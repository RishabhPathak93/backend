...
def extract_vulnerable_function(file_content: str, snippet: str):
    ...
    try:
        ...
        if idx != -1:
            logger.debug("Snippet matched using exact substring match")
        ...
        if m:
            logger.debug("Snippet matched using whitespace-flexible regex")
        ...
        if s_line is not None and score >= 0.6:
            logger.debug(f"Snippet matched using line-window similarity with score {score:.2f}")
            extracted = "\n".join(file_lines[s_line-1:e_line])
            return extracted, [s_line, e_line]

        # ---- 4) Give up
        logger.debug("Snippet matching failed, using fallback [1,1]")
        return snippet, [1, 1]
...
def extract_relevant_info(llm_output, file_name, scan_id, triggered_by, file_content):
    ...
    vulnerabilities = []
    logger.debug(f"Extracting findings from LLM output for {file_name}")
    ...
    logger.debug(f"Extracted {len(vulnerabilities)} vulnerabilities from LLM output for {file_name}")
