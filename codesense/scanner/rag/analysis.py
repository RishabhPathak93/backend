...
def scan_single_file(file_path, scan_id, triggered_by, max_chunk_batch=5, cache_enabled=True):
    ...
    async def analyze_chunk_batch(chunk_batch, session):
        ...
        for chunk in chunk_batch:
            ...
            if cache_enabled and key in retrieval_cache:
                ...
                logger.debug(f"Cache miss for chunk in {file_name}")

            logger.debug(f"LLM output for {file_name}: {output[:500]}...")  # Log first 500 chars
    ...
    try:
        ...
        if len(file_content.strip()) < 1:
            logger.info(f"Skipping {file_name}: file too small ({len(file_content.strip())} characters).")
            return []
